    ...
    def config_hw(self):
        ...
        self._run_seq(
            [
                ...
                #36 is below
                (const(0x36), b"\x48"),   #0x3A
                (_PIXFMT, b"\x66"),   #0x3A

                (const(0xB0), b"\x80"),   #Intf mode cntrl
                (_FRMCTR1, b"\xA0"),  # Frame rate ctrl
                (_INVCTR, b"\x02"),  #disp inverson cntrl
                (const(0xB6), b"\x02\x02"),  #display func cntrl     

                ( const(0xE9), b"\x00"),  #set Image cntrl
                #( const(0x53), b"\x28"),  #set Image cntrl
                #( const(0x51), b"\x7F"),  #set Image cntrl

                #(_VMCTR1, b"\x3E\x28"),  # VCOM ctrl 1
                #(_VMCTR2, b"\x86"),  # VCOM ctrl 2
                #(_VSCRSADD, b"\x00"),  # Vertical scrolling start address
                #(_GMCTRP1, bytes([0x00, 0x07, 0x0C, 0x05, 0x13, 0x09, 0x36, 0xAA, 0x46, 0x09, 0x0A, 0x0D, 0x1A, 0x1E, 0x0F])),  #xe0
                #(_GMCTRN1, bytes([0x00, 0x20, 0x23, 0x04, 0x0A, 0x06, 0x37, 0x56, 0x49, 0x04, 0x0C, 0x0A, 0x33, 0x37, 0x0F])),  #e1
